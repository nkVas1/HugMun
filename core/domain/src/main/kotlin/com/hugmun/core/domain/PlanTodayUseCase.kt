...
public class PlanTodayUseCase(
    ...
    public suspend operator fun invoke(): DayPlan {
        ...
        val anchor = planAnchor(today)

        val entries = listOfNotNull(vigilance, anchor)
    ...
    /**
     * «Якорь» appears only when something is actually due.
     *
     * Unlike the speed protocol, which the app schedules, this queue is generated by the
     * user's own items and their individual intervals. On a day with nothing due the
     * practice simply is not in the plan — there is no value in reviewing something that
     * is still fresh, and the spacing is the whole mechanism.
     */
    private suspend fun planAnchor(today: kotlinx.datetime.LocalDate): PlannedPractice? {
        val due = anchorRepository.dueItems(today).size
        if (due == 0) return null

        return PlannedPractice(
            practice = Practice.ANCHOR,
            dueOn = today,
            estimated = (ANCHOR_MINUTES_PER_ITEM * due).coerceAtLeast(2).minutes,
            reason = PlanReason.SCHEDULED,
            rationale = "Сегодня подошёл срок повторить $due " +
                when {
                    due % 10 == 1 && due % 100 != 11 -> "запись"
                    due % 10 in 2..4 && due % 100 !in 12..14 -> "записи"
                    else -> "записей"
                } + ".",
            isDue = true,
        )
    ...
    private companion object {
        ...
        /** Roughly how long one item takes, including the pause before a reveal. */
        const val ANCHOR_MINUTES_PER_ITEM = 1
    }
}
